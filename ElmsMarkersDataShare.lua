    ...
    if tag and data then
        if (tag == GetGroupLeaderUnitTag() and
            ...
            elseif dataIdentifier == 3 or dataIdentifier == 9 then
                ...
                tuple.wY = wY
                tuple.isAdd = isAdd
                if dataIdentifier == 9 then isRendezvous = true end
            end

            if tuple.zone and tuple.wX and tuple.wY and tuple.wZ and
                tuple.iconId and tuple.isAdd then
                if (isRendezvous) then
                    ElmsMarkers.PlaceRendezvousAt({
                        tuple.zone, tuple.wX, tuple.wY, tuple.wZ, tuple.iconId
                    }, false)
                elseif (tuple.isAdd == 2) then
                    ElmsMarkers.PlaceAtLocation({
                        tuple.zone, tuple.wX, tuple.wY, tuple.wZ, tuple.iconId
                    })
                else
                    ElmsMarkers.RemoveExactMarkerAt({
                        tuple.zone, tuple.wX, tuple.wY, tuple.wZ
                    })
                end
...
function ElmsMarkers.PlaceRendezvousAt(location, isLeader)
    ...
    local iconPlacement = OSI.CreatePositionIcon(wX, wY, wZ, texture,
                                                 iconSize * OSI.GetIconSize(),
                                                 {1, 1, 1}, 2.5, function(data)
    ...
    zo_callLater(function()
        ...
        ElmsMarkers.UI.announcementBannerLabel:SetText(
            "[Elms Markers] Sample text")
...
function ElmsMarkers.PrepareRendezvous()
    ...
    if (ElmsMarkers.lastPingTime == nil or
        ...
        if AreUnitsEqual(GetGroupLeaderUnitTag(), 'player') then
            ...
            ElmsMarkers.EncodeEnqueuePublish({zone, wX, wY, wZ, iconId}, true,
                                             true)
            ElmsMarkers.PlaceRendezvousAt({zone, wX, wY, wZ, iconId}, true)
        else
            CHAT_SYSTEM:AddMessage(
                "[ElmsMarkers] You must be the group lead to send group commands!")
        end
    else
        CHAT_SYSTEM:AddMessage(
            "[ElmsMarkers] You're sending group commands too quickly! Command not sent, try again later.")
    end
end
