...
function ElmsMarkers.setupUI()
    ...
    ui.removePublishButton =
        ElmsMarkers_Frame_Button_Group_Remove_Publish_Button
    ...
    ui.close:SetHandler("OnMouseUp", function() ui.frame:SetHidden(true) end,
                        "ElmsMarkers")
    ...
    ui.markerDropdown = ZO_ComboBox_ObjectFromContainer(
                            ElmsMarkers_Frame_Marker_Dropdown_Panel_Marker_Dropdown)
    ui.markerDropdown:SetSortsItems(false)
    ui.markerIcon:SetTexture(ElmsMarkers.iconData[ElmsMarkers.savedVars
                                 .selectedIconTexture])
    ...
    for k, v in pairs(markerEntries) do ui.markerDropdown:AddItem(v) end

    ui.markerDropdown:SetSelectedItemText(
        ElmsMarkers.reverseOptionMap[ElmsMarkers.savedVars.selectedIconTexture])

    ui.placeButton:SetHandler("OnMouseUp", ElmsMarkers.PlaceAtMe, "ElmsMarkers")
    ui.removeButton:SetHandler("OnMouseUp", ElmsMarkers.RemoveNearMe,
                               "ElmsMarkers")
    ui.placePublishButton:SetHandler("OnMouseUp", function()
        ElmsMarkers.PreparePublish(true)
    end, "ElmsMarkers")
    ui.removePublishButton:SetHandler("OnMouseUp", function()
        ElmsMarkers.PreparePublish(false)
    end, "ElmsMarkers")
    ElmsMarkers.CheckGroupLead()
end

function ElmsMarkers.SaveAnnouncementPosition()
    ElmsMarkers.savedVars.announcementOffsetX = ElmsMarkers.UI
                                                    .announcementBanner:GetLeft()
    ElmsMarkers.savedVars.announcementOffsetY = ElmsMarkers.UI
                                                    .announcementBanner:GetTop()
end

function ElmsMarkers.SetAnnouncementPosition()
    local x, y = ElmsMarkers.savedVars.announcementOffsetX,
                 ElmsMarkers.savedVars.announcementOffsetY
...
function ElmsMarkers.SetFramePosition()
    local x, y = ElmsMarkers.savedVars.frameOffsetX,
                 ElmsMarkers.savedVars.frameOffsetY
...
function ElmsMarkers.SetScale(element, value) element:SetScale(value) end
